...
typedef struct
{
    ...
    unsigned short      ctl;            //CCEE. CC - number of controller. EE - std effect
    unsigned short      ctl_val;        //XXYY. Value of controller
} sunvox_note;
...
int sv_rewind( int slot, int t ) SUNVOX_FN_ATTR;
int sv_volume( int slot, int vol ) SUNVOX_FN_ATTR;
int sv_send_event( int slot, int channel_num, int note, int vel, int module, int ctl, int ctl_val ) SUNVOX_FN_ATTR;
...
int sv_pattern_mute( int slot, int pat_num, int mute ) SUNVOX_FN_ATTR; //Use it with sv_lock_slot() and sv_unlock_slot()
...
typedef void* (*tsv_get_module_scope)( int slot, int mod_num, int channel, int* buffer_offset, int* buffer_size ) SUNVOX_FN_ATTR;
typedef unsigned int (*tsv_get_module_scope2)( int slot, int mod_num, int channel, signed short* read_buf, unsigned int samples_to_read ) SUNVOX_FN_ATTR;
...
typedef int (*tsv_pattern_mute)( int slot, int pat_num, int mute ) SUNVOX_FN_ATTR;
